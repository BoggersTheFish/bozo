...
A language model built on **sigmoid tension** instead of softmax attention — an implementation of the Thinking System (TS) theory of computation as constraint relaxation.

**Current focus: mathematical reasoning.** Formal mathematics is the ideal domain for a constraint-relaxation model — contradictions cannot exist by construction, so the constraint graph built during training is coherent by definition.
...
**Key finding:** Step 14,000 outperforms the final checkpoint. The second epoch of maths data partially overwrites the logic structure loaded in stage 1. This is a direct TS prediction: dense new constraints can displace existing coherent constraints. The new training run addresses this with `--logic_mix 0.10`.
...
- **Fused Triton kernel:** 64× memory reduction vs naive unfold, covers forward and backward
...
1. Does `--logic_mix 0.10` prevent the step-14k degradation? (Active investigation)
2. Does W=256 meaningfully improve multi-step proof following vs W=64?
3. What is the optimal logic mix ratio for stage 3?
4. Is the Chinchilla-optimal token count different for TS-native training vs standard cross-entropy?
